...
interface TaskViewerProps {
    ...
    setArrayObjects: (arrayObjects: { id: string; title: string; isChecked: boolean; tasks: { id: string; title: string; isChecked: boolean }[] }[]) => void;
}
        ...
        <div className="flex flex-col items-center mt-[15px]">
            {arrayObjects.map(task => (
                <div key={task.id} className="flex flex-col items-center justify-between p-4 w-full max-w-md mb-2 bg-gray-300">
                    ...
                    <TaskInput
                        onAddTask={(newTaskTitle) => handleAddSubtask(task.id, newTaskTitle)}
                    />
